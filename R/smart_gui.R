...
smart_gui <- function() {
  ...
  gbutton("Set Threshold", container = pro_g_top2, handler = function(h, ...) {
    ...
    gbutton(
      text = "\n   Set!   \n", container = up_fra,
      handler = function(...) {
        my_project$fleet$setSpecSettItm(
    ...
    thr_spin <- gslider(
      ...
      handler = function(...) {
        if (!is.null(my_project$fleet$specLogit[[svalue(spe_drop)]])) {
          my_project$fleet$setSpecLogitConf(
            selSpecie = svalue(spe_drop),
    ...
    maxb_spin <- gslider(
      ...
      handler = function(...) {
        ...
        print(ggplot_betasBoxplot(
          df_YearFGprod = my_project$fleet$betaMeltYear[[svalue(spe_drop)]],
  ...
  gbutton("Betas", container = pro_g_top2_ver, handler = function(h, ...) {
    ...
    Sys.sleep(1)
    my_project$setPlotBetaMeltYear(
      specie = svalue(spe_drop),
  ...
  gbutton("Production", container = pro_g_top2_ver, handler = function(h, ...) {
    ...
    Sys.sleep(1)
    my_project$setPlotProdMeltYear(
      specie = svalue(spe_drop),
      year = svalue(provie_drop)
    )
  ...
  gbutton("LFD Viewer", container = raw_g_top, handler = function(h, ...) {
    ...
    addSpring(spec_b)
    spec_drop <- gcombobox(
      items = my_project$specieInSurvey,
  ...
  gbutton("MEDITS index", container = raw_g_top, handler = function(h, ...) {
    ...
    medInd_b <- gbutton(
      ...
      handler = function(h, ...) {
        ...
        sex_sel <- sex_drop[1]
        tmp_abus <- data.frame(
          Class = my_project$surveyBySpecie[[specie_ind]]$abuAvg$Class,
    ...
    specie_drop <- gcombobox(
      items = my_project$specieInSurvey, selected = 1,
  ...
  gbutton("LFD Viewer", container = fis_g_top, handler = function(h, ...) {
    ...
    addSpring(spec_b)
    spec_drop <- gcombobox(
      items = my_project$specieInFishery,
      selected = 1, container = spec_b, expand = TRUE,
      editable = FALSE, handler = function(h, ...) {
  ...
  gbutton("Set Size Class", container = sim_g_top2, handler = function(h, ...) {
    if (is.null(my_project$fleet$ecoPrice)) {
      ...
      tmp_df <- my_project$fleet$ecoPrice[[unique(c(
        my_project$specieInFishery,
        my_project$specieInSurvey
      ))[1]]]
    }
  ...
  sim_Res_radio <- gradio(c("Summary", "Pattern", "Change"),
                          ...
                          handler = function(h, ...) {
                            ...
                            switch(svalue(sim_Res_radio),
                                   Summary = {},
                                   Pattern = {
                                     suppressWarnings(grid.arrange(my_project$simResPlot[["obsEffort"]],
                                   ...
                                   Change = {
                                     suppressWarnings(grid.arrange(my_project$simResPlot[["absChange"]],
