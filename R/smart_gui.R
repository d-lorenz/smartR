...
smart_gui <- function(){
  ...
  gbutton("Load", container = pro_g_top, handler = function(h,...){
    ...
    if(!is.null(my_project$rawDataSurvey)){
      ...
      svalue(raw_l1) <- paste("Specie: ", paste(my_project$specieInSurvey,
                                                collapse = " - "))
      svalue(raw_l3) <- paste("Years: from",
  ...
  gbutton("Save", container = pro_g_top, handler = function(h,...){
    save_dest <- gfile(text = "Select file name and destination directory",
                       type = "save",
  ...
  addSpring(pro_g_mid)
  grid_g <- gframe(text = "Environment", horizontal = FALSE,
                   container = pro_g_mid)
  n_cell_g <- glabel("   ---", container = grid_g)
  addSpring(grid_g)
  grid_b <- gbutton(text = "Show data", container = grid_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 2
  })
  ...
  addSpring(effo_g)
  effo_b <- gbutton(text = "Show data", container = effo_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 3
  })
  ...
  addSpring(pro_g_mid)
  figr_g <- gframe(text = "Fishing Ground", horizontal = FALSE,
                   container = pro_g_mid)
  addSpring(figr_g)
  figr_b <- gbutton(text = "Show data", container = figr_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 4
  })
  ...
  addSpring(pro_g_mid)
  regi_g <- gframe(text = "Register", horizontal = FALSE,
                   container = pro_g_mid)
  addSpring(regi_g)
  regi_b <- gbutton(text = "Show data", container = regi_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 5
  })
  ...
  addSpring(pro_g_mid)
  prod_g <- gframe(text = "Production", horizontal = FALSE,
                   container = pro_g_mid)
  addSpring(prod_g)
  prod_b <- gbutton(text = "Show data", container = prod_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 6
  })
  ...
  addSpring(samp_g)
  samp_b <- gbutton(text = "Show data", container = samp_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 7
  })
  ...
  addSpring(fish_g)
  fish_b <- gbutton(text = "Show data", container = fish_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 8
  })
  ...
  addSpring(mixt_g)
  mixt_b <- gbutton(text = "Show data", container = mixt_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 9
  })
  ...
  addSpring(pro_g_mid)
  simu_g <- gframe(text = "Simulation", horizontal = FALSE,
                   container = pro_g_mid)
  addSpring(simu_g)
  simu_b <- gbutton(text = "Show data", container = simu_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 11
  })
  ...
  addSpring(pro_g_mid)
  asse_g <- gframe(text = "Assessment", horizontal = FALSE,
                   container = pro_g_mid)
  addSpring(asse_g)
  asse_b <- gbutton(text = "Show data", container = asse_g,
                    handler = function(h,..){
    svalue(uti_gn) <- 12
  })
  ...
  pro_p <- ggraphics(container = pro_g, width = 550, height = 250,
                     expand = TRUE)
  ...
  gbutton(" Save ", container = gri_g_top1_dep, handler = function(h,...){
    ...
    tryCatch(expr = {
      if(!is.null(my_project$sampMap$gridBathy)){
        Sys.sleep(1)
        tmpOutBathy <- gfile(text = "Select output data name", type = "save",
        ...
        Sys.sleep(1)
        my_project$sampMap$saveGridBath(tmpOutBathy)
      }
    },
    error = function(error_message){
      message("An error has occurred!")
  ...
  addSpring(gri_g_top)
  gri_g_top2 <- gframe(text = "Environmental Asset", horizontal = TRUE,
                       container = gri_g_top)
  ...
  gbutton("Import", container = gri_g_top2, handler = function(h,...){
    ...
    tryCatch(expr = {
      dev.set(dev.list()[pre_dev+2])
      Sys.sleep(1)
      tmpInEnvfiles <- gfile(text = "Select Environment Asset file",
                             type = "open",
                             initial.filename = NULL, initial.dir = getwd())
  ...
  gri_p <- ggraphics(container = gri_g, width = 550, height = 250,
                     expand = TRUE)
  ...
  gbutton("from vmsbase DB", container = eff_g_top1, handler = function(h,...){
    ...
    tryCatch(expr = {
      ...
      pathLab <- glabel(text = paste0("Loaded DB:\t\t",
      ...
      gbutton(text = "Run Query", container = up_g, handler = function(...){
        ...
        Sys.sleep(1)
        my_project$loadFleeEffoDbs(effort_path = tmpVMSfiles,
  ...
  eff_g_top1b <- gframe(text = "Fishing Point", horizontal = TRUE,
                        container = eff_g_top)
  ...
  gbutton("Set", container = eff_g_top1b, handler = function(h,...){
    ...
    tryCatch(expr = {
      ...
      addSpring(up_fra)
      spe_fra <- gframe(text = "Speed Range", container = up_fra,
                        horizontal = TRUE)
      spe_lay <- glayout(homogeneous = FALSE, spacing = 10,
                         container = spe_fra)
      spe_lay[1,1, anchor = 0] <- "Min"
      spe_lay[2,1, anchor = 0] <- "Max"
      spe_lay[1,2, anchor = 0] <- gspinbutton(from = 0, to = 30, by = 1,
      ...
      spe_lay[2,2, anchor = 0] <- gspinbutton(from = 0, to = 30, by = 1,
      ...
      addSpring(up_fra)
      dep_fra <- gframe(text = "Depth Range", container = up_fra,
                        horizontal = TRUE)
      dep_lay <- glayout(homogeneous = FALSE, spacing = 10,
                         container = dep_fra)
      dep_lay[1,1, anchor = 0] <- "Min"
      dep_lay[2,1, anchor = 0] <- "Max"
      dep_lay[1,2, anchor = 0] <- gspinbutton(from = -5000, to = 0, by = 10,
                                              value = -500,
                                              container = dep_lay,
                                              handler = function(...){
                                                my_project$fleet$plotSpeedDepth(which_year = svalue(yea_drop),
      ...
      dep_lay[2,2, anchor = 0] <- gspinbutton(from = -5000, to = 0, by = 10,
      ...
      addSpring(up_fra)
      yea_fra <- gframe(text = "Year View", container = up_fra,
                        horizontal = TRUE)
      ...
      gbutton(text = "\n   Set!   \n", container = up_fra,
        ...
        plot(NULL, xlim = c(0,5), ylim = c(0,1), axes = FALSE,
             xlab = "", ylab = "")
        ...
        text(1,0, labels = "Parameters", pos = 3, col = "grey20", srt = 45,
             offset = 1.5)
        svalue(int_progBar) <- 10
        Sys.sleep(1)
        my_project$fleet$setFishPoin()
        points(2,0, pch = 19, col = "grey20")
        text(2,0, labels = "Fishing Points", pos = 3, col = "grey20", srt = 45,
             offset = 1.5)
        svalue(stat_bar) <- ""
        ...
        text(3,0, labels = "Cell Assign", pos = 3, col = "grey20", srt = 45,
             offset = 1.5)
      ...
      addSpring(up_fra)
      fipo_gra <- ggraphics(width = 600, height = 400, container = up_g,
                            expand = TRUE)
      ...
      visible(temp_dia) <- TRUE
      my_project$fleet$plotSpeedDepth(which_year = svalue(yea_drop),
                                      speed_range = unlist(lapply(spe_lay[1:2,2],
                                                                  svalue)),
                                      depth_range = unlist(lapply(dep_lay[1:2,2],
                                                                  svalue)))
    },
    error = function(error_message){
      message("An error has occurred!")
  ...
  eff_g_top2 <- gframe(text = "Maps", horizontal = TRUE, container = eff_g_top,
                       expand = TRUE)
  addSpace(eff_g_top2, 10)
  effvie_drop <- gcombobox(items = "Year", selected = 1,
  ...
  eff_g_top3 <- gframe(text = "Effort Asset", horizontal = TRUE,
                       container = eff_g_top)
  ...
  eff_p <- ggraphics(container = eff_g, width = 550, height = 250,
                     expand = TRUE)
  
  
  ####   Fishing Grounds   ####
  
  fig_g <- ggroup(horizontal = FALSE, container = uti_gn,
                  label = "Fishing Grounds")
  fig_g_top <- gframe(horizontal = TRUE, container = fig_g)
  
  addSpring(fig_g_top)
  
  fig_g_top_dist <- gframe(text = "Distance Metric", horizontal = FALSE,
                           container = fig_g_top, expand = TRUE)
  addSpring(fig_g_top_dist)
  fg_metr <- gcombobox(c("euclidean", "manhattan", "maximum",
                         "binary", "canberra", "minkowski"), selected = 1,
                       editable = FALSE,
  ...
  fig_g_top_clus <- gframe(text = "Clustering", horizontal = TRUE,
                           container = fig_g_top, expand = TRUE)
  ...
  fig_g_top_plot <- gframe(text = "Plot", horizontal = TRUE,
                           container = fig_g_top, expand = TRUE)
  ...
  fg_plotCut <- gcombobox(items = 2:50,
                          ...
                          handler = function(h,...){
                            ...
                            suppressWarnings(grid.arrange(my_project$sampMap$ggIchFGlin,
                                                          ...
                                                          layout_matrix = rbind(c(1,3,3,5,6,7),
                                                                                c(2,4,4,5,6,7))))
  ...
  gbutton("    Select\nPartitioning", container = fig_g_top,
          handler = function(h,...){
  ...
  fig_g_top3 <- gframe(text = "Fishing Ground Asset", horizontal = TRUE,
                       container = fig_g_top)
  ...
  gbutton("Import", container = fig_g_top3, handler = function(h,...){
    tryCatch(expr = {
      Sys.sleep(1)
      tmpInEffFiles <- gfile(text = "Select Fishing Ground Asset file",
                             type = "open",
                             initial.filename = NULL, initial.dir = getwd())
      svalue(stat_bar) <- "Loading..."
      Sys.sleep(1)
  ...
  fisGro_p <- ggraphics(container = fig_g, width = 550, height = 250,
                        expand = TRUE)
  ...
  gbutton("Load EU register", container = reg_g_top_raw,
          handler = function(h,...){
    ...
    ggplot_registerDispatch(curRegister = my_project$fleet$rawRegister,
                            selPlot = "Summary")
  ...
  addSpring(reg_g_top_raw)
  gbutton("View Raw Data", container = reg_g_top_raw,
          handler = function(h,...){
    temp_flee <- gbasicdialog(title="Explore Fleet Register",
                              parent = main_win)
  ...
  addSpring(reg_g_top_raw)
  addSpring(reg_g_top)
  reg_g_top_view <- gframe(text = "Plot Summary Data", horizontal = TRUE,
  ...
  sel_regSet <- gradio(c("All", "Vms"), selected = 1, horizontal = FALSE,
                       container = reg_g_top_view, handler = function(h,...){
                         ...
                         if(svalue(sel_regSet) == "All"){
                           ...
                           ggplot_registerDispatch(curRegister = my_project$fleet$vmsRegister,
                                                   selPlot = svalue(sel_regPlot))
                         }
                       })
  addSpace(reg_g_top_view, 10)
  sel_regPlot <- gcombobox(c("Summary", "Main Gear",
  ...
  addSpace(reg_g_top_view, 10)
  addSpring(reg_g_top)
  reg_g_top_harbs <- gframe(text = "Harbour Distance", horizontal = TRUE,
                            container = reg_g_top, expand = TRUE)
  addSpace(reg_g_top_harbs, 10)
  gbutton("Get Harbours", container = reg_g_top_harbs,
          handler = function(h,...){
  ...
  gimage(system.file("ico/folder-man.png", package="smartR"),
         ...
         handler = function(h,...){
           ...
           load_path <- gfile(text = "Select Harbour List file", type = "open",
                              filter = list("R files" = list(patterns = c("*.rData"))))
  ...
  gbutton("Set Weighted\nDistance", container = reg_g_top_harbs,
          handler = function(h,...){
    dev.set(dev.list()[pre_dev+5])
  ...
  addSpace(reg_g_top_harbs, 10)
  addSpring(reg_g_top)
  regGro_p <- ggraphics(container = reg_g, width = 550, height = 250,
                        expand = TRUE)
  ...
  spe_fra <- gframe(text = "Specie", container = pro_g_top1, horizontal = TRUE,
                    expand = TRUE)
  ...
  gbutton("Set Threshold", container = pro_g_top2, handler = function(h,...){
    ...
    addSpace(up_fra, 20)
    spe_fra <- gframe(text = "Specie", container = up_fra, horizontal = TRUE,
                      expand = TRUE)
    addSpring(spe_fra)
    ...
    addSpring(spe_fra)
    addSpace(up_fra, 20)
    thr_fra <- gframe(text = "Threshold", container = up_fra,
                      horizontal = TRUE)
    addSpace(thr_fra, 20)
    thr_spin <- gspinbutton(from = 0, to = 100,
                            ...
                            handler = function(...){
                              ...
                              hist(tmp_spe[tmp_spe <= svalue(max_x_spin)],
                                   ...
                                   xlab = "")
                              abline(v = svalue(thr_spin), col = 2, lwd = 3,
                                     lty = 2)
                            })
    addSpace(up_fra, 20)
    addSpace(thr_fra, 20)
    
    bou_fra <- gframe(text = "Limits and Breaks", container = up_fra,
                      horizontal = FALSE)
    addSpace(bou_fra, 20)
    ...
    num_bre_spin <- gspinbutton(from = 10, to = 1000,
                                ...
                                handler = function(...){
                                  ...
                                  hist(tmp_spe[tmp_spe <= svalue(max_x_spin)],
                                       ...
                                       main = bquote(italic(.(svalue(spe_drop)))),
                                       xlab = "")
                                  abline(v = svalue(thr_spin), col = 2,
                                         lwd = 3, lty = 2)
                                })
    addSpace(bou_fra, 20)
    addSpring(up_fra)
    ...
    addSpace(up_fra, 20)
    addSpring(up_g)
    land_gra <- ggraphics(width = 600, height = 400, container = up_g,
                          expand = TRUE)
    visible(temp_dia) <- TRUE
  ...
  gbutton("Get Logit", container = pro_g_top2, handler = function(h,...){
    ...
    addSpace(up_fra, 20)
    mod_fra <- gframe(text = "Model", container = up_fra, horizontal = TRUE,
                      expand = TRUE)
    addSpace(mod_fra, 20)
    mod_radSel <- gradio(items = c("GLM", "CART", "RF"),
                         selected = 1, horizontal = FALSE, container = mod_fra,
                         handler = function(...){
                           switch(svalue(mod_radSel),
                                  GLM = {
                                    lapply(list(par_modSel[1,2]),
                                           function(x) enabled(x) <- TRUE)
                                    lapply(par_modSel[2:3,2],
                                           function(x) enabled(x) <- FALSE)},
                                  CART = {
                                    lapply(list(par_modSel[2,2]),
                                           function(x) enabled(x) <- TRUE)
                                    lapply(par_modSel[c(1,3),2],
                                           function(x) enabled(x) <- FALSE)},
                                  RF = {
                                    lapply(list(par_modSel[3,2]),
                                           function(x) enabled(x) <- TRUE)
                                    lapply(par_modSel[1:2,2],
                                           function(x) enabled(x) <- FALSE)}
                           )
                         })
    addSpace(mod_fra, 20)
    ...
    addSpace(up_fra, 20)
    par_fra <- gframe(text = "Parameters", container = up_fra,
    ...
    par_modSel[1,1:2] <- ""
    par_modSel[2,1] <- "cp"
    par_modSel[2,2] <- gspinbutton(from = 0, to = 1, by = 0.001, value = 0.02,
                                   container = par_modSel)
    par_modSel[3,1] <- "CV"
    par_modSel[3,2] <- gspinbutton(from = 2, to = 10, by = 1, value = 2,
                                   container = par_modSel)
    ...
    thr_fra <- gframe(text = "Tune Cutoff", container = up_fra, expand = TRUE,
                      horizontal = TRUE)
    addSpace(thr_fra, 20)
    thr_spin <- gslider(from = 0.01, to = 0.99,
                        ...
                        handler = function(...){
                          if(!is.null(my_project$fleet$specLogit[[svalue(spe_drop)]])){
                            my_project$fleet$setSpecLogitConf(selSpecie = svalue(spe_drop),
    ...
    addSpring(bot_lef_g)
    tmp_txt <- gtext(text = NULL, width = 300, height = 350,
                     container = bot_lef_g)
    addSpring(bot_lef_g)
    addSpace(bot_g, 10)
    logi_gra <- ggraphics(width = 300, height = 400, container = bot_g,
                          expand = TRUE)
    visible(temp_dia) <- TRUE
    addSpace(bot_g, 10)
  ...
  gbutton("Get NNLS", container = pro_g_top2, handler = function(h,...){
    ...
    gbutton(text = " Get\nNNLS", container = up_fra, handler = function(...){
      ...
      tryCatch(expr = {
        if (is.null(my_project$fleet$effoProdAllLoa)) {
          my_project$fleet$setEffoProdAllLoa()
        }
        my_project$getNnlsModel(specie = svalue(spe_drop),
        ...
        svalue(tmp_txt) <- paste("\n\nRaw Scenarios:\n\n\t",
                                 ...
                                 "%)\n\n",
                                 sep = "")
        my_project$fleet$plotNNLS(specie = svalue(spe_drop),
    ...
    addSpace(up_fra, 20)
    obs_fra <- gframe(text = "Min Observations", container = up_fra,
    ...
    thr_fra <- gframe(text = "R2 Threshold", container = up_fra,
                      expand = TRUE, horizontal = TRUE)
    addSpace(thr_fra, 20)
    ...
    bot_lef_g <- ggroup(horizontal = FALSE, container = bot_g)
    addSpring(bot_lef_g)
    tmp_txt <- gtext(text = NULL, width = 200, height = 350,
                     container = bot_lef_g)
    addSpring(bot_lef_g)
    addSpace(bot_g, 10)
    nnls_gra <- ggraphics(width = 300, height = 400, container = bot_g,
                          expand = TRUE)
    visible(temp_dia) <- TRUE
    addSpace(bot_g, 10)
  ...
  gbutton("Tune\nBetas", container = pro_g_topBeta, handler = function(h,...){
    ...
    addSpring(spe_fra)
    addSpace(up_fra, 20)
    maxb_fra <- gframe(text = "Max Beta", container = up_fra, expand = TRUE,
                       horizontal = TRUE)
    addSpace(maxb_fra, 20)
    maxb_spin <- gslider(from = 1,
                         to = ceiling(max(my_project$fleet$betaMeltYear[[svalue(spe_drop)]]$Productivity)),
    ...
    add(set_gru, logi_sta)
    print(ggplot_betasBoxplot(df_YearFGprod = my_project$fleet$betaMeltYear[[svalue(spe_drop)]],
                              int_hline = max(my_project$fleet$betaMeltYear[[svalue(spe_drop)]]$Productivity)))
  })
  addSpring(pro_g_topBeta)
  addSpring(pro_g_top)
  ...
  gbutton("  Predict\nProduction", container = pro_g_top3,
          handler = function(h,...){
    enabled(pro_g_top) <- FALSE
    Sys.sleep(1)
  ...
  addSpring(pro_g_top)
  pro_g_top2_view_g <- ggroup(horizontal = FALSE, container = pro_g_top,
                              expand = TRUE)
  pro_g_top2_view <- gframe(text = "View", horizontal = TRUE,
                            container = pro_g_top2_view_g, expand = TRUE)
  addSpring(pro_g_top2_view)
  provie_drop <- gcombobox(items = "Year   ", selected = 1,
  ...
  gbutton("Betas", container = pro_g_top2_ver, handler = function(h,...){
    ...
    Sys.sleep(1)
    my_project$setPlotBetaMeltYear(specie = svalue(spe_drop),
  ...
  gbutton("Production", container = pro_g_top2_ver, handler = function(h,...){
    ...
    Sys.sleep(1)
    my_project$setPlotProdMeltYear(specie = svalue(spe_drop),
                                   year = svalue(provie_drop))
  ...
  addSpring(pro_g_top2_ver)
  addSpring(pro_g_top2_view)
  gbutton("    Total\nProduction", container = pro_g_top2_view,
          handler = function(h,...){
  ...
  addSpring(pro_g_top)
  proGro_p <- ggraphics(container = pro_g, width = 550, height = 250,
                        expand = TRUE)
  ...
  gbutton("Load Sample", container = raw_g_top1, handler = function(h,...){
    ...
    if(!is.null(my_project$rawDataSurvey)){
      ...
      if(is.null(my_project$specieInFishery)){
        ...
        assSpe_drop[] <- intersect(my_project$specieInSurvey,
  ...
  gbutton("LFD Viewer", container = raw_g_top, handler = function(h,...){
    ...
    temp_dia <- gwindow(title="Survey Length Frequency Distribution Viewer",
                        visible = FALSE,
    ...
    lfdfra_g <- gframe("LFD data", horizontal = TRUE, container = pop_g_top,
                       expand = TRUE)
    addSpring(lfdfra_g)
    
    spec_b <- gframe("Specie", horizontal = FALSE, container = lfdfra_g,
                     expand = TRUE)
    addSpring(lfdfra_g)
    addSpring(spec_b)
    ...
    addSpring(spec_b)
    sex_b <- gframe("Sex", horizontal = FALSE, container = lfdfra_g,
                    expand = TRUE)
    addSpring(lfdfra_g)
    addSpring(sex_b)
    ...
    pop_p <- ggraphics(container = pop_g, width = 750, height = 450,
                       expand = TRUE)
    addSpring(pop_g_top)
  ...
  addSpring(raw_g_top)
  gbutton("Spatial Distribution", container = raw_g_top,
          handler = function(h,...){
    
    temp_dia <- gwindow(title="Spatial Distribution of Survey sampling",
                        visible = FALSE,
    ...
    pop_g_top <- gframe(horizontal = TRUE, container = pop_g, spacing = 10)
    addSpring(pop_g_top)
    spec_b <- gframe("Specie", horizontal = FALSE, container = pop_g_top,
                     expand = TRUE)
    
    addSpring(spec_b)
    spec_drop <- gcombobox(items = as.character(my_project$specieInSurvey),
                           selected = 1,
    ...
    spec_drop$set_size(value = c(width = 150))
    addSpring(spec_b)
    sex_b <- gframe("Sex", horizontal = FALSE, container = pop_g_top,
                    expand = TRUE)
    addSpring(pop_g_top)
    addSpring(sex_b)
    sex_drop <- gcombobox(items = c("Female", "Male", "Unsex"),
    ...
    pop_p <- ggraphics(container = pop_g, width = 650, height = 450,
                       expand = TRUE)
    visible(temp_dia) <- TRUE
  ...
  gbutton("MEDITS index", container = raw_g_top, handler = function(h,...){
    ...
    comfra_g <- gframe("Compute", horizontal = TRUE, container = med_g_top,
                       expand = TRUE)
    addSpring(comfra_g)
    
    strata_f <- gframe("Strata", horizontal = FALSE, container = comfra_g,
                       expand = TRUE)
    depth_b <- gbutton(text = "Set", container = strata_f,
                       handler = function(h,...){
    ...
    areas_f <- gframe("Areas", horizontal = FALSE, container = comfra_g,
                      expand = TRUE)
    area_b <- gbutton(text = "Set", container = areas_f,
                      handler = function(h,...){
    ...
    medInd_f <- gframe("MEDITS Index", horizontal = FALSE,
                       container = comfra_g, expand = TRUE)
    medInd_b <- gbutton(text = "Set", container = medInd_f,
                        handler = function(h,...){
    ...
    addSpring(comfra_g)
    addSpring(med_g_top)
    plofra_g <- gframe("Show", horizontal = TRUE, container = med_g_top,
                       expand = TRUE)
    addSpace(plofra_g, 10)
    specie_drop <- gcombobox(items = my_project$specieInSurvey, selected = 1,
    ...
    addSpace(plofra_g, 10)
    sex_drop <- gcombobox(items = c("Female", "Male", "Unsex", "All"),
    ...
    addSpace(plofra_g, 10)
    addSpring(med_g_top)
    pop_p <- ggraphics(container = med_g, width = 550, height = 250,
                       expand = TRUE)
  ...
  gbutton("Load Sample", container = fis_g_top1, handler = function(h,...){
    ...
    if(!is.null(my_project$rawDataFishery)){
      ...
      svalue(fis_l1) <- paste("Specie: ", paste(my_project$specieInFishery,
                                                collapse = " - "))
      svalue(fis_l3) <- paste("Years: from",
  ...
  gbutton("LFD Viewer", container = fis_g_top, handler = function(h,...){
    temp_dia <- gwindow(title="Fishery Length Frequency Distribution Viewer",
                        visible = FALSE,
                        parent = main_win, width = 800, height = 500)
  ...
  addSpring(fis_g_top)
  gbutton("Spatial Distribution", container = fis_g_top,
          handler = function(h,...){
    temp_dia <- gwindow(title="Spatial Distribution of Fishery sampling",
    ...
    spec_b <- gframe("Specie", horizontal = FALSE, container = pop_g_top,
                     expand = TRUE)
    addSpring(spec_b)
    spec_drop <- gcombobox(items = as.character(my_project$specieInFishery),
                           selected = 1,
                           container = spec_b, editable = FALSE,
                           handler = function(h,...){
                             spe_ind <- which(my_project$specieInFishery == svalue(spec_drop))
  ...
  cont_g <- gframe("Mixture Analysis", horizontal = TRUE,
                   container = mix_g_top, expand = TRUE)
  addSpring(cont_g)
  sourceMix_r <- gradio(items = c("Survey", "Fishery"), horizontal = FALSE,
  ...
  spec_mix_f <- gframe("Specie and Sex", horizontal = FALSE,
                       container = cont_g, expand = TRUE)
  addSpring(spec_mix_f)
  spec_drop_mix <- gcombobox(items = "Specie", selected = 1,
  ...
  addSpring(spec_mix_f)
  sex_drop_mix <- gcombobox(items = c("Female", "Male", "Unsex"), selected = 1,
                            container = spec_mix_f, editable = FALSE,
                            expand = TRUE)
  addSpring(spec_mix_f)
  addSpring(cont_g)
  ...
  addSpring(ncoh_f)
  ncih_sb <- gspinbutton (from = 1, to = 100, by = 1, value = 3, digits = 0,
                          container = ncoh_f)
  addSpring(ncoh_f)
  ...
  addSpring(gcurv_f)
  gcurv_r <- gradio(items = c("von Bertalanffy", "Gompertz"),
                    horizontal = FALSE, container = gcurv_f, expand = TRUE)
  addSpring(gcurv_f)
  
  addSpring(cont_g)
  
  go_g <- gframe("MCMC sim", horizontal = TRUE, container = cont_g,
                 expand = TRUE)
  addSpring(go_g)
  go_g_ada <- ggroup(horizontal = FALSE, container = go_g, expand = TRUE)
  addSpring(go_g_ada)
  glabel("N. adapt: ", container = go_g_ada)
  mc_niter <- gcombobox(c(100, 1000, 5000, 10000), selected = 2,
  ...
  mc_nsamp <- gcombobox(c(100, 1000, 5000, 10000), selected = 2,
  ...
  addSpring(cont_g)
  view_g <- gframe("View", horizontal = TRUE, container = cont_g,
                   expand = TRUE)
  ...
  addSpring(cont_g)
  addSpace(mix_g_top, 20)
  mix_p <- ggraphics(container = mix_g, width = 550, height = 250,
                     expand = TRUE)
  ...
  cohofra_g <- gframe("Cohort data", horizontal = TRUE,
                      container = cohoP_g_top,
                      expand = TRUE)
  addSpring(cohofra_g)
  sourceCoh_r <- gcombobox(items = c("Survey", "Fishery"), horizontal = FALSE,
  ...
  cohSpe_b <- gframe("Specie", horizontal = FALSE, container = cohofra_g,
                     expand = TRUE)
  addSpring(cohofra_g)
  addSpring(cohSpe_b)
  spec_drop_coh <- gcombobox(items = "Specie", selected = 1,
  ...
  cohSex_b <- gframe("Sex", horizontal = FALSE, container = cohofra_g,
                     expand = TRUE)
  addSpring(cohofra_g)
  addSpring(cohSex_b)
  ...
  cohCoh_b <- gframe("Cohort", horizontal = FALSE, container = cohofra_g,
                     expand = TRUE)
  addSpring(cohofra_g)
  addSpring(cohCoh_b)
  cohCoh_drop <- gcombobox(items = "Age", selected = 1, container = cohCoh_b,
                           editable = FALSE, handler = function(h,...){
  ...
  addSpring(cohofra_g)
  addSpace(cohoP_g_top, 2)
  cohPop_p <- ggraphics(container = cohoP_g, width = 550, height = 250,
                        expand = TRUE)
  ...
  sim_g_Effo <- gframe(text = "Effort Index", horizontal = FALSE,
                       container = sim_g_top)
  ...
  sim_g_SeaDays <- gframe(text = "Days At Sea", horizontal = FALSE,
                          container = sim_g_top)
  ...
  sim_g_Prod <- gframe(text = "Production Index", horizontal = FALSE,
                       container = sim_g_top)
  ...
  gbutton("Set Cost Data", container = sim_g_top2, handler = function(h,...){
    ...
    cost_g <- ggroup(horizontal = FALSE, container = tempWind_Cost,
                     spacing = 15)
    cost_g_top <- gframe(horizontal = TRUE, container = cost_g, spacing = 20)
    
    addSpring(cost_g_top)
    gbutton("   Load\nCost Data", container = cost_g_top,
            handler = function(h,...){
    ...
    addSpring(cost_g_top)
    gbutton("         Get\nCost Regression", container = cost_g_top,
            handler = function(h,...){
    ...
    addSpring(cost_g_top)
    gbutton("         Set\nCost Prediction", container = cost_g_top,
            handler = function(h,...){
    ...
    cost_p <- ggraphics(container = cost_g, width = 550, height = 250,
                        expand = TRUE)
  ...
  gbutton("Set Size Class", container = sim_g_top2, handler = function(h,...){
    ...
    if(is.null(my_project$fleet$ecoPrice)){
      tmp_df <- data.frame(Class = c("Small", "Large"),
                           Units = factor(x = c("Length", "Length"),
      ...
      tmp_df <- my_project$fleet$ecoPrice[[unique(c(my_project$specieInFishery,
                                                    my_project$specieInSurvey))[1]]]
    }
    ...
    gain_g <- ggroup(horizontal = FALSE, container = tempWind_Gain,
                     spacing = 15)
    gain_g_top <- gframe(horizontal = TRUE, container = gain_g, spacing = 20)
    
    addSpace(gain_g_top, 15)
    sel_specie <- gcombobox(items = unique(c(my_project$specieInFishery,
    ...
    addSpace(gain_g_top, 15)
    add_class <- gbutton(text = "Add Size Class", container = gain_g_top,
                         handler = function(...){
    ...
    set_data <- gbutton(text = "Set Data", container = gain_g_top,
                        handler = function(...){
      my_project$fleet$setEcoPrice(sel_specie = svalue(sel_specie),
                                   price_df = cost_df[])
    })
    addSpring(gain_g_top)
    ioButt_g <- ggroup(horizontal = FALSE, container = gain_g_top,
                       expand = TRUE)
    gimage(system.file("ico/document-save-2.ico", package="smartR"),
           container = ioButt_g)
    gimage(system.file("ico/folder-man.png", package="smartR"),
           container = ioButt_g)
    addSpring(gain_g_top)
    close_butt <- gbutton(text = "Close", container = gain_g_top,
                          handler = function(...){
  ...
  addSpring(sim_g_top2)
  gbutton("Set length weight\nrelationship", container = sim_g_top2,
          handler = function(h,...){
    ...
    lwRel_g <- ggroup(horizontal = TRUE, container = tempWind_LWrel,
    ...
    assfra_g <- gframe("Input setup", horizontal = FALSE,
    ...
    assSou_r <- gradio(c("Survey", "Fishery"), selected = 1,
    ...
    assSpe_drop <- gcombobox(items = "Specie", selected = 1,
    ...
    gbutton("\t  Load\nWeighted Sample", container = assfra_g,
            handler = function(h,...){
      ...
      print(
        ggplot() +
          ...
          annotate("text", vjust = 1, hjust = 0, size = 7,
                   ...
                   label = paste("alpha == ",
                                 svalue(valu_lyt[1,2]), sep = "")) +
          annotate("text", vjust = 1, hjust = 0, size = 7,
                   x = min(lw_data$Length) + 5,
                   y = quantile(lw_data$Weight, 0.999) - 10, parse = TRUE,
                   label = paste("beta == ",
                                 svalue(valu_lyt[2,2]), sep = "")) +
    ...
    lwRel_f_valu <- gframe(text = "Values", horizontal = TRUE,
    ...
    addSpace(lwRel_g, 10)
    lwRel_p <- ggraphics(container = lwRel_g, width = 550, height = 550,
                         expand = TRUE)
    addSpace(lwRel_g, 10)
  ...
  sim_g_Sim <- gframe(text = "Scenario", horizontal = TRUE,
  ...
  sim_f_Thr <- gframe(text = "Threshold", horizontal = TRUE,
                      container = sim_g_SimPar2)
  sim_Thr <- gslider(from = 0, to = 100, by = 1, value = 10,
                     container = sim_f_Thr)
  addSpring(sim_g_SimPar2)
  sim_f_Mode <- gframe(text = "Effort Pattern Mode", horizontal = TRUE,
  ...
  sim_f_Sca <- gframe(text = "Time Scale", horizontal = TRUE,
                      container = sim_g_SimPar3)
  addSpring(sim_f_Sca)
  sim_Sca <- gradio(items = c("Year", "Season"), selected = 1,
  ...
  # addSpring(sim_f_Den)
  addSpring(sim_g_SimPar3)
  sim_Ban <- gbutton("Set Closed Area", container = sim_g_SimPar3,
                     handler = function(h,...){
    ...
    print(my_project$sampMap$gooMapPlot +
            geom_polygon(aes_(x = ~long, y = ~lat, group = ~group,
                              fill = ~Banned),
    ...
    addHandlerChanged(obj = close_df, , handler = function(...){
      if(sum(which(!my_project$simBanFG$Banned == close_df[]$Banned)) > 0){
        ...
        cat("\nChanged FG ", idChan, ": ",
            ...
            as.character(close_df[]$Banned[idChan]), sep = "")
  ...
  addSpring(sim_g_SimPar)
  addSpring(sim_g_Sim)
  gbutton("   Start\nSimulation", container = sim_g_Sim,
          handler = function(h,...){
    dev.set(dev.list()[pre_dev+9])
    ...
    my_project$genSimEffo(method = svalue(sim_Mode),
                          areaBan = as.numeric(my_project$simBanFG$FG[my_project$simBanFG$Banned == "Banned"]))
    my_project$simulateFishery(thr0 = svalue(sim_Thr),
  ...
  addSpace(sim_g_top, 10)
  sim_Res <- gframe("View", horizontal = TRUE, container = sim_g_top,
                    expand = TRUE)
  sim_Res_radio <- gradio(c("Summary", "Pattern", "Change"), selected = 1,
                          ...
                          handler = function(h,...){
                            ...
                            switch(svalue(sim_Res_radio),
                                   Summary = {},
                                   Pattern = {suppressWarnings(print(grid.arrange(my_project$simResPlot[["obsEffort"]],
                                                                                  my_project$simResPlot[["optEffort"]],
                                                                                  layout_matrix = rbind(c(1,1,2,2),
                                                                                                        c(1,1,2,2)))))},
                                   Change = {suppressWarnings(print(grid.arrange(my_project$simResPlot[["absChange"]],
  ...
  sim_p <- ggraphics(container = sim_g, width = 550, height = 250,
                     expand = TRUE)
  ...
  assSM_rad <-  gradio(items = c("Single", "Multi"), selected = 1,
                       horizontal = FALSE,
  ...
  assSpe_drop <- gcombobox(items = "Specie", selected = 1, editable = FALSE,
                           container = ass_g_spePred,
                           handler = NULL)
  assSpe_drop$set_size(value = c(width = 150))
  
  addSpring(ass_g_spePred)
  assPre_but <- gbutton(text = "Set Interaction", container = ass_g_spePred,
                        handler = function(h,...){
    ...
    iteIte <- matrix(data = "None", nrow = length(speLst),
                     ncol = length(speLst), byrow = TRUE)
    iteItePar <- matrix(data = 0, nrow = length(speLst),
                        ncol = length(speLst), byrow = TRUE)
    ...
    topAss_g <- ggroup(horizontal = FALSE, container = toptopAss_g,
                       expand = TRUE)
    ...
    for(oneSpe in 1:length(speLst)){
      speWid[[oneSpe]] <- list()
      speWid[[oneSpe]]$fra <- gframe(text = speLst[oneSpe], horizontal = FALSE,
                                     container = topAss_g)
      addSpace(speWid[[oneSpe]]$fra, 10)
      speWid[[oneSpe]]$gruA <- ggroup(horizontal = TRUE,
                                      container = speWid[[oneSpe]]$fra)
      addSpring(speWid[[oneSpe]]$gruA)
      glabel(text = "Type:", container = speWid[[oneSpe]]$gruA)
      speWid[[oneSpe]]$rad <- gradio(items = c("None", "Predator", "Prey"),
                                     horizontal = TRUE,
                                     ...
                                     handler = function(h,...){
                                       ...
                                       speGra <- make_empty_graph(n = length(speLst)) %>%
                                         ...
                                         add_vertices(nv = 1,
                                                      label = "Fishing\nFleet") %>%
                                         add_edges(c(rbind(length(speLst)+1,
                                                           1:length(speLst))),
                                                   color = "Black", lty = 1)
                                       plot(speGra, main = "Interaction Graph",
                                            edge.arrow.size = 0.5,
                                            edge.width = 2,
                                            vertex.color = c(vertColo,
                                                             "White"),
                                            vertex.size = c(rep(60,
                                                                length(vertColo)),
                                                            90),
                                            layout = nodeLay,
      ...
      for(eatSpe in 1:length(speLst)){
        ...
        speWid[[oneSpe]]$diet[[eatSpe]]$fraBrad <- gradio(items = c("None",
                                                          ...
                                                          handler = function(h,...){
                                                            ...
                                                            vertColo <- as.character(vertColDF[match(iteTyp,
                                                                                                     vertColDF[,1]),2])
                                                            firWhi <- which(iteIte != "None",
                                                                            arr.ind = TRUE)
                                                            edgeLty <- rep(1,
      ...
      speWid[[oneSpe]]$gruC <- ggroup(horizontal = TRUE,
                                      container = speWid[[oneSpe]]$fra)
      addSpring(speWid[[oneSpe]]$gruC)
      glabel(text = "Chi:", container = speWid[[oneSpe]]$gruC)
      speWid[[oneSpe]]$gruCeChi <- gedit(text = "0.0", horizontal = TRUE,
                                         width = 5,
                                         container = speWid[[oneSpe]]$gruC)
      
      glabel(text = "Omega:", container = speWid[[oneSpe]]$gruC)
      speWid[[oneSpe]]$gruCeOm <- gedit(text = "0.0", horizontal = TRUE,
                                        width = 5,
                                        container = speWid[[oneSpe]]$gruC)
    ...
    addSpace(topAss_g, 10)
    addSpace(toptopAss_g, 10)
    int_p <- ggraphics(container = toptopAss_g, width = 350, height = 250,
                       expand = TRUE)
    addSpace(toptopAss_g, 10)
    ...
    speGra <- make_empty_graph(n = length(speLst)) %>%
      ...
      add_edges(c(rbind(length(speLst)+1, 1:length(speLst))), color = "Black",
                lty = 1)
  ...
  addSpring(ass_g_spePred)
  addSpring(ass_g_top)
  ass_Fore <- gframe("Forecast\nNext Year", horizontal = FALSE,
                     container = ass_g_top, expand = TRUE)
  addSpring(ass_Fore)
  ass_Fore_radio <- gradio(c("No", "Yes"), selected = 1, horizontal = TRUE,
                           container = ass_Fore)
  addSpring(ass_Fore)
  addSpace(ass_g_top, 10)
  assParS_but <- gbutton(text = "Set Input ", container = ass_g_top,
                         handler = function(h,...){
    if(svalue(assSM_rad) == "Single"){
      tmpSpe <- svalue(assSpe_drop)
      my_project$setAssessData(specie = tmpSpe,
      ...
      parZbef_f <- gframe(text = "Z before Maturity", horizontal = TRUE,
                          container = topAss_g)
      ...
      for(oneSpe in intersect(my_project$specieInFishery,
                              my_project$specieInSurvey)){
        my_project$setAssessData(specie = oneSpe,
                                 forecast = ifelse(svalue(ass_Fore_radio) == "No",
                                                   FALSE, TRUE))
        ...
        parZbef_f <- gframe(text = "Z before Maturity", horizontal = TRUE,
                            container = topAss_g)
        addSpace(parZbef_f, 25)
        parZbef_ge <- gedit(text = parZbef_df, width = 15,
                            container = parZbef_f)
        addSpace(parZbef_f, 25)
        addSpace(topAss_g, 10)
  ...
  addSpace(ass_g_top, 10)
  assParR_but <- gbutton(text = "Inspect Input", container = ass_g_top,
                         handler = function(h,...){
    ...
    for(s in 1:length(names(my_project$assessData))){
      assign(paste0("Tab", s), ggroup(horizontal = FALSE, container = ass_gn,
                                      label = names(my_project$assessData)[s]))
      assign(paste0("Txt", s),
             gtext(text = paste(capture.output(print(my_project$assessData[[names(my_project$assessData)[s]]])),
  ...
  addSpace(ass_g_top, 10)
  ass_Str_but <- gbutton(text = "Start", container = ass_g_top,
                         handler = function(h,...){
    if(svalue(assSM_rad) == "Single"){
  ...
  addSpring(ass_g_top)
  ass_Res <- gframe("View", horizontal = TRUE, container = ass_g_top,
                    expand = TRUE)
  addSpace(ass_Res, 10)
  assRes_drop <- gcombobox(items = "    Specie", selected = 1,
  ...
  ass_p <- ggraphics(container = ass_g, width = 550, height = 250,
                     expand = TRUE)
